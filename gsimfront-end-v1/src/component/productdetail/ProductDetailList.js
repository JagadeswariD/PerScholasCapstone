...
const ProductDetailList = () => {
   ...
   const deleteProductDetail = async (Id) => {
        try{
             await ProductDetailService.deleteProductDetail(Id).then((res) => {
    ...
    return(
     <div>
            <div>
               { show &&
                <ProductToast setShow={setShow} header="Success" message="Product Detail deleted successfully!!!"/>}
            </div>
     <Card>
      ...
      <div className="displayGrid">
      {
          ...
          :
           currentItems.map((productDetails) =>
             <Card style={{ width: '15rem', height: '25rem' }} key={productDetails.id}>
                  <Card.Img style={{ width: '13rem', height: '10rem' }}  variant="top" src={window.location.origin + '/img/'+productDetails.category.categoryName+'/'+productDetails.product.productName+'.png'} />
                  <Card.Body>
